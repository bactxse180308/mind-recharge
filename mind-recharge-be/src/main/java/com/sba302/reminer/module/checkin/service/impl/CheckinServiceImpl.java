...
class CheckinServiceImpl implements CheckinService {
    ...
    public CheckinResponse upsertToday(Long userId, CheckinRequest request) {
        // Lock the user row so concurrent "first check-in of the day" requests
        // for the same user cannot race into duplicate inserts.
    ...
    private CheckinResponse toResponse(DailyCheckin c) {
        String responseText = contentCatalogService.getText(ContentType.MOOD_RESPONSE, c.getResponseKey());
